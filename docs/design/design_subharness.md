...
- **설계 당시(mvp/03) 코드의 조잡함 원인 — 역사 기록(아래 파일들은 현 레포에 부재, 이 설계가 해소한 것)**
  ...
  - → **현 구현 상태**: grounding은 core/grounding.py(`build_corpus`·`find_ungrounded`)로 구현되어 S2b 생성(backend/app/gateway/harness_design.py:351-369)·critic 게이트(:258-268) 양쪽에서 사용. 단계 승격=STEPS S0~S3+done 상태머신(:28)으로 구현 완료. 레퍼런스 주입·크리틱 패스도 구현(§③).
...
S0 셋업     plan.md 파싱 → 디자인 토큰 lock(palette·font·grid·aspect)
            ...
            + aspect 미기재 시 material_matrix 추론 폴백 + languages 정규화(:492-527)
            + 소재 매트릭스(채널 × 언어) 산출
...
S2 컴포넌트   2a 비주얼 : Gemini가 배경/키비주얼 생성 (텍스트 free)
   빌드                   — 이미지 생성 실패 시 placeholder 그라데이션 폴백(:332-346)
...
S3 Final    백엔드 _s3_final은 metadata.md만 생성(scene 합성 없음, 아래 상세)
...
- **final scene 조립 주체(spec §9-3)**: 백엔드 S3(`_s3_final`, backend/app/gateway/harness_design.py:434-473)는 **metadata.md만 생성 — scene을 합성하지 않는다**. final scene 조립=**프론트 `CockpitProvider.assembleScenes`**(frontend/components/cockpit/CockpitProvider.tsx:340-368, design done 시 일괄 :473-485)가 layout.spec.json을 읽어 전 언어 수행, `design/final/{lang}/main.scene`으로 PUT /vfs 저장(scene-wins: 수동 편집한 언어는 스킵). **cross-actor 계약**: ReviewHarness `_collect_scene_copy`(harness_review.py:218-249)가 main.scene `{version, objects}`의 textbox에서 role→text로 카피를 복원, 없으면 layout.spec.json[copy][lang] 폴백. scene 조립 로직은 frontend/lib/sceneAssembler.ts:49-73.
- grounding 검증은 core/grounding.py(`build_corpus`·`find_ungrounded`)로 구현 — S2b 생성(harness_design.py:351-369)·critic 게이트(:258-268) 양쪽 사용, factsheet에 없는 금리·만기 토큰 차단. (설계 당시 "generate.py 로직 재사용" 인용은 mvp/03 배경 — 역사 기록.)
- 단계 승격: 설계 당시 mvp/03 `generate.py`의 rough/comp/final(정적 번들)을 실제 단계로 승격한다는 목표는 STEPS S0~S3+done 상태머신(harness_design.py:28)으로 구현 완료.

### confirm 정책 (단계 게이트 + bypass)

- **(1) 게이트 정지** = `GateEnvelope(kind="confirm", step, critic, auto_advanced, actions=["confirm","regenerate"])`(harness_design.py:230-243). 승인 action=`advance|confirm`(:167-185). 게이트 대상=GATED_STEPS(S1·S2a·S2b·S2c·S3), S0은 비게이트.
- **(2) bypass**: DesignStudio **Setting**(Refactor 27줄 사이드바)에서 게이트 **단위 토글**(예: Rough만 확인, 나머지 자동). bypass 시 critic 품질게이트는 **S1·S3(7항목 시각 critic)·S2b(grounding ungrounded 수치)만 — S2a/S2c는 critic 없음(항상 통과)**(GATED_STEPS·CRITIC_STEPS :30-33, `_critic_gate` :245-269).
- **(3) 자동 재생성**: bypass에서 critic 실패 시 step당 1회 한도로 재생성, 2차 실패 시 meta.warnings 기록 후 진행(:206-218).
- **(4) done**: gate 없음 + meta.auto_advanced(자동 통과 단계 목록, :193-203).
- **AskUser 훅과의 관계**: design은 ask 게이트 없음(design의 게이트는 kind=confirm만) — ask는 brainstorming 전용. design state의 `pending_ask` 키는 잔재(읽는 코드 없음 — @harness_audit.md §2).
...
- 디자인 토큰·레퍼런스 라이브러리의 **출처와 큐레이션 방식** → **채택**: backend/app/references/design/*.json — `_load_references`(harness_design.py:289-296)가 S1 프롬프트 references 블록에 주입(:306-314).
- 크리틱 패스 **루브릭의 구체 항목** → **확정**: `_critic_gate` 단계표(:245-269) — S1·S3=7항목 시각 critic(hierarchy/grid/whitespace/cta/compliance/copy_visual/brand), S2b=grounding ungrounded 수치, S2a/S2c=critic 없음.
