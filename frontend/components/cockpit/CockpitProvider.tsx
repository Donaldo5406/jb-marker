  ...
  const sendChat = useCallback(
    async (p: { prompt: string; provider: Provider; isMarker: boolean }) => {
      ...
      try {
        ...
        // brainstorming은 syncGate=false — pendingGate는 위 applyGate가 이미 권위 있게 세팅했으므로
        // _state.json에서 재설정하면 AskUserToast가 재마운트되어 '같은 팝업이 두 번' 깜빡인다.
        await Promise.all([refreshTree(), loadManifest(id),
          activeStudio === "brainstorming" ? loadBrainState(id, false) : Promise.resolve()]);
