...
def _prompt_for(state: str) -> str:
    ...
    prompt += (
        ...
        "없는 완전한 빈 배경입니다.\n"
        "- 좌상단 로고 예약 영역을 **별도의 흰 박스·패널·섹터로 구분해 그리지 마세요** — "
        "배경이 그대로 자연스럽게 이어지되 그 구역만 요소 없이 밝게 유지하세요(4차 실측: "
        "백판이 '따로 얹은 느낌'을 만듦).\n"
        "- 배경의 차트·스크린·그래프에는 숫자·퍼센트·글자를 일절 넣지 마세요(흐릿한 잔글씨 "
        "포함 금지) — 곡선과 막대 형태만.")
    if "#FFD166" in prompt:
        prompt += ("\n- 헤드라인 전체를 **실제 붓으로 쓴 한글 붓글씨 캘리그래피**로 렌더하세요 "
                   "— 필압 변화·거친 획 끝·번짐이 보이는 브러시 스트로크, 골드(#FFD166). "
                   "고딕/산세리프/금박 인쇄체 절대 금지(3차 실측: 금박 고딕으로 이탈).")
    return prompt
