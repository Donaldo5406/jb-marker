...
class Settings:
    ...
    # S2a 이미지 생성 모델 — usage 기록·google_client 호출의 단일 출처.
    # gemini-3-pro-image: 한글 텍스트 베이크 정확(2.5-flash는 한글 깨짐 — 실측 확인).
    # GOOGLE_IMAGE_MODEL 환경변수로 오버라이드(비용 절감 시 gemini-3.1-flash-image 등).
    google_image_model: str = "gemini-3-pro-image"
    # M7-A Supabase
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        google_image_model=os.getenv("GOOGLE_IMAGE_MODEL", "gemini-3-pro-image"),
