...
def reviewed_image_for(asset_id: str, lang: str | None,
                       render_langs: frozenset = frozenset(),
                       primary_lang: str | None = None) -> str:
    ...
    2026-07-05 poc_E: 합성 렌더가 하나도 없을 때(붕괴) 전 언어 verdict가 v1.png 히어로
    하나에 겹쳐, 비주 언어(en/vi/zh) bbox가 주 언어(ko) 포스터의 엉뚱한 위치에 그려졌다
    (예: vi 헤드라인 y가 ko '정기예금'에 얹힘). primary_lang이 주어지면 **주 언어·언어없음
    (RC 등)만 v1.png 히어로에 남기고**, 비주 언어는 자기 합성 경로로 분리한다(합성 부재 시
    그 이미지는 히어로가 아니어서 표시 안 됨) → 히어로엔 주 언어 위반 + RC만 정확히 표시.
    """
    a = asset_id or ""
    ...
    if lang and primary_lang and lang != primary_lang:
        return f"review/_render/{lang}.png"
...
def resolve_location(location: dict, *, asset_id: str, lang: str | None,
                     ...
                     render_langs: frozenset = frozenset(),
                     primary_lang: str | None = None) -> dict:
    ...
    (합성 렌더는 v1과 같은 3:4 정규화 좌표계 — background 슬롯 풀블리드).
    primary_lang: 합성 부재 시 비주 언어를 주 언어 히어로에서 분리(poc_E 정리).
    """
    out = dict(location)
    image = reviewed_image_for(asset_id, lang, render_langs, primary_lang)
    out["image"] = image
