...
PERSONA = (
    "당신은 금융 마케팅 시니어 영상 디렉터입니다. 서사 비트(훅·혜택·신뢰·CTA)·"
    "샷 길이·카메라 무빙·전환·자막 타이밍·컴플라이언스 톤에 능하며, 텍스트는 절대 "
    "footage 픽셀에 굽지 않고 레이어로 분리합니다. 콘티는 구조화 JSON으로만 출력합니다."
)

# [V1 Storyboard] 지시·JSON 예시
V1_INSTR = (
    "\n\n[V1 Storyboard] 아래 토큰·비트를 참고해 storyboard(JSON)를 출력하세요. "
    "shots 배열의 각 항목은 id·start·end(초)·footage_prompt(텍스트-free 배경)·camera·"
    "transition_in·transition_out·layers를 갖습니다. layers의 각 항목은 role·copy_key·"
    "in·out(초)·anim·font_px·color(#RRGGBB)·bbox{x,y,w,h}를 갖습니다. "
    ...
    '"footage_prompt":"추상 금융 성장, 텍스트 없음","camera":"slow zoom-in",'
